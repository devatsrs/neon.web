	...
	select
	 ...
	 -- testing output
		...
		ELSE
			...
			SET
			rtd.EndDate = IF(drtr.EndDate='0000-00-00 00:00:00',NULL,drtr.EndDate),


			rtd.OneOffCost = CASE WHEN ( rtd.OneOffCostCurrency is not null)
			THEN
			...
			END ,
			rtd.MonthlyCost = CASE WHEN ( rtd.MonthlyCostCurrency is not null)  -- (MonthlyCost * p_months) as MonthlyCost,
			THEN
			...
			rtd.CostPerCall = CASE WHEN ( rtd.CostPerCallCurrency is not null)
			THEN
			...
			rtd.CostPerMinute = CASE WHEN ( rtd.CostPerMinuteCurrency is not null)
			THEN
			...
			rtd.SurchargePerCall = CASE WHEN ( rtd.SurchargePerCallCurrency is not null)
			THEN
			...
			rtd.SurchargePerMinute = CASE WHEN ( rtd.SurchargePerMinuteCurrency is not null)
			THEN
			...
			rtd.OutpaymentPerCall = CASE WHEN ( rtd.OutpaymentPerCallCurrency is not null)
			THEN
			...
			rtd.OutpaymentPerMinute = CASE WHEN ( rtd.OutpaymentPerMinuteCurrency is not null)
			THEN
			...
			rtd.Surcharges = CASE WHEN ( rtd.SurchargesCurrency is not null)
			THEN
			...
			rtd.Chargeback = CASE WHEN ( rtd.ChargebackCurrency is not null)
			THEN
			...
			rtd.CollectionCostAmount = CASE WHEN ( rtd.CollectionCostAmountCurrency is not null)
			THEN
			...
			rtd.CollectionCostPercentage = CASE WHEN ( rtd.CollectionCostAmountCurrency is not null)
			THEN
			...
			rtd.RegistrationCostPerNumber = CASE WHEN ( rtd.RegistrationCostPerNumberCurrency is not null)
			THEN
			...
			rtd.OneOffCostCurrency = drtr.OneOffCostCurrency,
			rtd.MonthlyCostCurrency = drtr.MonthlyCostCurrency,
			rtd.CostPerCallCurrency = drtr.CostPerCallCurrency,
			rtd.CostPerMinuteCurrency = drtr.CostPerMinuteCurrency,
			rtd.SurchargePerCallCurrency = drtr.SurchargePerCallCurrency,
			rtd.SurchargePerMinuteCurrency = drtr.SurchargePerMinuteCurrency,
			rtd.OutpaymentPerCallCurrency = drtr.OutpaymentPerCallCurrency,
			rtd.OutpaymentPerMinuteCurrency = drtr.OutpaymentPerMinuteCurrency,
			rtd.SurchargesCurrency = drtr.SurchargesCurrency,
			rtd.ChargebackCurrency = drtr.ChargebackCurrency,
			rtd.CollectionCostAmountCurrency = drtr.CollectionCostAmountCurrency,
			rtd.RegistrationCostPerNumberCurrency = drtr.RegistrationCostPerNumberCurrency,

			rtd.updated_at = now() ,
			rtd.ModifiedBy = p_ModifiedBy    ,
