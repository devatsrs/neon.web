...
ALTER TABLE `tblVendorConnection` ADD COLUMN `Location` VARCHAR(200) NULL AFTER `AuthenticationMode`;
ALTER TABLE speakIntelligentRoutingEngine.`tblRoutingProfileRate`
	ADD COLUMN `Location` VARCHAR(200) NULL DEFAULT NULL AFTER `TimezoneId`;
	ALTER TABLE speakIntelligentRoutingEngine.`tblVendorRate`
	ADD COLUMN `Location` VARCHAR(200) NULL DEFAULT NULL AFTER `CategoryName`;