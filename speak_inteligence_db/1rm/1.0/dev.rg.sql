...
GenerateRateTable:BEGIN
		...
		insert into tmp_Raterules_dup2 (			rateruleid ,		OriginationCode ,		OriginationDescription ,code ,		description ,		RowNo 		,   `Order`)
			select rateruleid ,		OriginationCode ,		OriginationDescription,		code ,		description ,		RowNo, `Order` from tmp_Raterules_;
		...
		THEN
			...
			INSERT INTO tmp_VendorCurrentRates1_
				...
				FROM (
							 SELECT  vt.AccountId,tblAccount.AccountName, r2.Code as OriginationCode, r2.Description as OriginationDescription,tblRate.Code, tblRate.Description,
		...
		ELSE
			...
			INSERT INTO tmp_VendorCurrentRates1_
				Select DISTINCT AccountId,AccountName,OriginationCode,OriginationDescription,Code,Description, Rate, RateN,ConnectionFee,EffectiveDate,TrunkID,TimezonesID,CountryID,RateID,Preference
				FROM (
 							 SELECT  vt.AccountId,tblAccount.AccountName, r2.Code as OriginationCode, r2.Description as OriginationDescription,tblRate.Code, tblRate.Description,
		...
		Select AccountId,AccountName,OriginationCode,OriginationDescription,Code,Description, Rate, RateN,ConnectionFee,EffectiveDate,TrunkID,TimezonesID,CountryID,RateID,Preference
		FROM (
					 SELECT * ,
		...
		THEN
			...
			INSERT INTO tmp_VendorCurrentRates_GroupBy_
				Select AccountId,max(AccountName),max(OriginationCode),OriginationDescription,max(Code),Description,max(Rate),max(RateN),max(ConnectionFee),max(EffectiveDate),TrunkID,TimezonesID,max(CountryID),max(RateID),max(Preference)
				FROM
				(
					Select AccountId,AccountName,OriginationCode,OriginationDescription,r.Code,r.Description, Rate, RateN,ConnectionFee,EffectiveDate,TrunkID,TimezonesID,r.CountryID,r.RateID,Preference
				...
				INSERT INTO tmp_VendorCurrentRates_ (AccountId,AccountName,OriginationCode,OriginationDescription,Code,Description, Rate, RateN,ConnectionFee,EffectiveDate,TrunkID,TimezonesID,CountryID,RateID,Preference)
			  		SELECT AccountId,AccountName,OriginationCode,OriginationDescription,Code,Description, Rate, RateN,ConnectionFee,EffectiveDate,TrunkID,TimezonesID,CountryID,RateID,Preference
		...
		DO
			...
			INSERT INTO tmp_Rates2_ (OriginationCode,OriginationDescription,code,description,rate,rateN,ConnectionFee)
				select  OriginationCode,OriginationDescription,code,description,rate,rateN,ConnectionFee from tmp_Rates_;
				...
				THEN
						...
						inner JOIN tmp_Raterules_ rr ON rr.RateRuleId = v_rateRuleId_ and
																(
																	 ( rr.OriginationCode != '' AND r.Code LIKE (REPLACE(rr.OriginationCode,'*', '%%')) )
																	 OR
																	 ( rr.OriginationDescription != '' AND r.Description LIKE (REPLACE(rr.OriginationDescription,'*', '%%')) )
																 )
																 AND
																 (
						...
						left JOIN tmp_Raterules_dup rr2 ON rr2.Order > rr.Order and
																		 (
																			 ( rr2.OriginationCode != '' AND r.Code LIKE (REPLACE(rr2.OriginationCode,'*', '%%')) )
																			 OR
																			 ( rr2.OriginationDescription != '' AND r.Description LIKE (REPLACE(rr2.OriginationDescription,'*', '%%')) )
																		 )
																		AND
																		(
						...
						(
							SELECT
								...
								CASE WHEN p_GroupBy = 'Desc'  THEN
													@rank := CASE WHEN ( @prev_Description = vr.Description  AND @prev_Rate <=  vr.Rate AND ROUND(((vr.Rate - @prev_Rate) /( @prev_Rate * 100)),2) > v_percentageRate  ) THEN @rank+1
													 ELSE
														 1
													 END

								ELSE	@rank := CASE WHEN ( @prev_RowCode = vr.RowCode  AND @prev_Rate <=  vr.Rate  AND ROUND(((vr.Rate - @prev_Rate) /( @prev_Rate * 100)),2) > v_percentageRate ) THEN @rank+1
							...
							SELECT
								...
								CASE WHEN p_GroupBy = 'Desc'  THEN
										...
										@preference_rank := CASE WHEN (@prev_Description  = vr.Description  AND @prev_Preference > vr.Preference  )   THEN @preference_rank + 1
																		WHEN (@prev_Description  = vr.Description  AND @prev_Preference = vr.Preference AND @prev_Rate <= vr.Rate  AND ROUND(((vr.Rate - @prev_Rate) /( @prev_Rate * 100)),2) > v_percentageRate ) THEN @preference_rank + 1

																		ELSE 1 END
								ELSE
												@preference_rank := CASE WHEN (@prev_Code  = vr.RowCode  AND @prev_Preference > vr.Preference  )   THEN @preference_rank + 1
																		WHEN (@prev_Code  = vr.RowCode  AND @prev_Preference = vr.Preference AND @prev_Rate <= vr.Rate   AND ROUND(((vr.Rate - @prev_Rate) /( @prev_Rate * 100)),2) > v_percentageRate ) THEN @preference_rank + 1
			...
			THEN
				...
				THEN
					...
					INSERT INTO tmp_Vendorrates_stage3_
						select  vr.OriginationCode ,vr.OriginationDescription ,  vr.RowCode as RowCode ,vr.description , vr.rate as rate , vr.rateN as rateN , vr.ConnectionFee as  ConnectionFee
				...
				INSERT IGNORE INTO tmp_Rates_ (OriginationCode ,OriginationDescription,code,description,rate,rateN,ConnectionFee,PreviousRate)
                SELECT
				OriginationCode,
				OriginationDescription,
				RowCode,
