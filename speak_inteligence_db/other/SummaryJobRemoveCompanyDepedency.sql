...
CREATE PROCEDURE `prc_insertResellerData`(
	...
	IN `p_invoiceTo` TEXT,
	IN `p_FooterTerm` TEXT,
	IN `p_LogoAS3Key` TEXT,
	IN `p_LogoUrl` TEXT,
	IN `p_TermsCondition` TEXT,
	IN `p_invoiceFrom` TEXT
...
BEGIN
	...
	#UPDATE tblCompany AS t1,
	#	(SELECT CurrencyId, TimeZone FROM tblAccount WHERE AccountID=p_accountid) AS t2
	#	SET t1.CurrencyId = t2.CurrencyId,
	#	    t1.TimeZone = t2.TimeZone
	#	WHERE CompanyID=p_childcompanyid;
	...
	IF p_is_product =1
	THEN	
		INSERT INTO speakintelligentBilling.tblProduct (CompanyId,Name,Code,Description,Amount,Active,Note,CreatedBy,ModifiedBy,created_at,updated_at)
		...
		FROM speakintelligentBilling.tblProduct
		WHERE CompanyId = p_companyid AND FIND_IN_SET(ProductID,p_product);
	END IF;
	IF p_is_subscription = 1
	THEN
		
		INSERT INTO speakintelligentBilling.tblBillingSubscription(`CompanyID`,Name,Description,InvoiceLineDescription,ActivationFee,created_at,updated_at,ModifiedBy,CreatedBy,CurrencyID,AnnuallyFee,QuarterlyFee,MonthlyFee,WeeklyFee,DailyFee,Advance)
		...
		FROM speakintelligentBilling.tblBillingSubscription
		WHERE CompanyID = p_companyid AND FIND_IN_SET(SubscriptionID,p_subscription);
	END IF;
	
	INSERT INTO tblReseller(ResellerName,CompanyID,ChildCompanyID,AccountID,FirstName,LastName,Email,Password,Status,AllowWhiteLabel,InvoiceTo,FooterTerm,LogoUrl,LogoAS3Key,TermsAndCondition,InvoiceFrom,created_at,updated_at,created_by)
	SELECT p_accountname as ResellerName,p_companyid as CompanyID,p_childcompanyid as ChildCompanyID,p_accountid as AccountID,p_firstname as FirstName,p_lastname as LastName,p_email as Email,p_password as Password,'1' as Status,p_allowwhitelabel as AllowWhiteLabel,p_invoiceTo as InvoiceTo,p_FooterTerm as FooterTerm,p_LogoUrl as LogoUrl,p_LogoAS3Key as LogoAS3Key,p_TermsCondition as TermsAndCondition,p_invoiceFrom as InvoiceFrom,Now(),Now(),'system' as created_by;
