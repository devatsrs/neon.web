...
CREATE INDEX `IX_ID` ON `RMCDR3`.`tblVendorCDR`(`ID`);

CREATE INDEX `IX_ID` ON `RMCDR3`.`tblVendorCDRFailed`(`ID`);

DROP PROCEDURE IF EXISTS `prc_DeleteDuplicateUniqueID2`;
DELIMITER |
CREATE PROCEDURE `prc_DeleteDuplicateUniqueID2`(IN `p_CompanyID` INT, IN `p_CompanyGatewayID` INT, IN `p_ProcessID` VARCHAR(200), IN `p_tbltempusagedetail_name` VARCHAR(200))
BEGIN
