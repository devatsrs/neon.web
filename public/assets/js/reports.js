...
function getAnalysisData(chart_type,submitdata){
    ...
    $.ajax({
        ...
        dataType: 'json',
        data:submitdata+'&chart_type='+chart_type,
        aysync: true,
...
function reloadCharts(table_id,pageSize,$searchFilter){
    ...
    getAnalysisData($('.tab-pane.active').find("input[name='chart_type']").val(),$('.tab-pane.active').find('form').serialize());

    /* get data by time in bar chart*/
    loadBarChart($('.tab-pane.active').find("input[name='chart_type']").val(),$('.tab-pane.active').find('form').serialize());
