        ...
        switch ($JobType) {
            case 'AI':
                ...
                if ($JobID = Job::insertGetId($data)) {
                    RateSheetHistory::AddtoHistory($JobID,'AI',$data);
            ...
            case 'SI':
                ...
                if ($JobID = Job::insertGetId($data)) {
                    RateSheetHistory::AddtoHistory($JobID,'SI',$data);
            ...
            case 'CDI':
                ...
                if ($JobID = Job::insertGetId($data)) {
                    RateSheetHistory::AddtoHistory($JobID,'CDI',$data);
            ...
            case 'CDU':
                ...
                $data["JobStatusID"] = isset($jobStatus[0]->JobStatusID) ? $jobStatus[0]->JobStatusID : '';
                $data["JobLoggedUserID"] = User::get_userID();
                if(!empty($options['translationname'])){
                    $translationname = $options['translationname'];
                }else{
                    $translationname = '';
                }
                $data["Title"] =  $translationname;
                $data["Description"] = isset($jobType[0]->Title) ? $jobType[0]->Title : '';
                $data["CreatedBy"] = User::get_user_full_name();
                $data["updated_at"] = date('Y-m-d H:i:s');
                $data["created_at"] = date('Y-m-d H:i:s');
            ...
            case 'DIFT':
                /*
                 *  Data Import From Template File Upload Job Log
                 */
                $rules = array(
                    'CompanyID'         => 'required',
                    'JobTypeID'         => 'required',
                ...
                /*
                 * Double check if file is uploaded correctly before logging job
                 * */
                $rules = array(
                    'FileName' => 'required',
                    'FilePath' => 'required',
            ...
            case 'CSRI':
                /*
                 *  Compared Special Rate Import Job Log
                 */
                $rules = array(
                    'CompanyID'         => 'required',
                    'JobTypeID'         => 'required',
            ...
            case 'TRO':
                /*
                 *  Termination Rate Operations (update/delete) (approve)
                 */
                $rules = array(
                    'CompanyID' => 'required',
                    'JobTypeID' => 'required',
                ...
                if($OperationType=='Update'){
                    $data["Title"] = 'Termination Rate Update ('.$options['RateTableName'].')';
                }elseif($OperationType=='Delete'){
                    $data["Title"] = 'Termination Rate Delete ('.$options['RateTableName'].')';
                }elseif($OperationType=='Approve'){
                    $data["Title"] = 'Termination Rate Approve ('.$options['RateTableName'].')';
                }elseif($OperationType=='Reject'){
                    $data["Title"] = 'Termination Rate Reject ('.$options['RateTableName'].')';
                }else{
                    $data["Title"] = (isset($jobType[0]->Title) ? $jobType[0]->Title : '');
                }
                $data["Description"]    = ' ' . isset($jobType[0]->Title) ? $jobType[0]->Title : '';
            ...
            case 'GE':
                ...
                if ($JobID = Job::insertGetId($data)) {
                    ...
                    return array("status" => "failed", "message" => "Problem Creating Job.");
                }
