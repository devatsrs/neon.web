...
<script>
    $(document).ready(function ($) {
        ...
        $('#clone-template-form').submit(function(e){
            e.preventDefault();
            update_new_url = baseurl + '/email_template/clone';
            var formdata = new FormData(($('#clone-template-form')[0]));
            formdata.append('LanguageID', $("#clone-template-form [name='LanguageID']").val());
            formdata.append('SystemType', $("#clone-template-form [name='SystemType']").val());

            showAjaxScript(update_new_url, formdata, function(response){
                $(".btn").button('reset');
                if (response.status == 'success') {
                    $('#clone-modal-template').modal('hide');
        ...
        $('#clone-modal-template').on('shown.bs.modal', function(event){
            var modal = $(this);  
        ...
        $('#clone-modal-template').on('hidden.bs.modal', function(event){
            var modal = $(this);
            modal.find('.message').show();
            popup_type = 0;
        $("#clone-template-form #email_from").val('').trigger('change');
        });
...
@section('footer_ext')
    @parent
<div class="modal fade" id="clone-modal-template">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <form id="clone-template-form" method="post">
