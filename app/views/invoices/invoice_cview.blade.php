    ...
    <div class="x-content">
      <div class="x-row">
        <div class="x-span8">
          ...
          <div class="amount"> <span class="overdue"><?php if($Invoice->InvoiceStatus==Invoice::PAID){echo number_format($payment_log['paid_amount'],$Account->RoundChargesAmount);}elseif($Invoice->InvoiceStatus!=Invoice::PAID && $payment_log['paid_amount']>0){echo number_format($payment_log['due_amount'],$Account->RoundChargesAmount);}else{echo number_format($payment_log['total'],$Account->RoundChargesAmount);}  ?></span> </div>
        </div>
