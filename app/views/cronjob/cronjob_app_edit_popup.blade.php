<script type="text/javascript">
    ...
    $('table tbody').on('click','.delete-config',function(ev){
        ...
        if(result){
            submit_ajax(baseurl+'/cronjobs/delete/'+$(this).attr('data-id') + '/app');
        }
    });
    ...
    $('#add-new-config-form').submit(function(e){
        ...
        if( typeof CronJobID != 'undefined' && CronJobID != ''){
            update_new_url = baseurl + '/cronjobs/update/'+CronJobID + '/app';
        }else{
            update_new_url = baseurl + '/cronjobs/create/app';
        }
        $.ajax({
