                                ...
                                <div class="pull-right">
                                    @if(count($taggedUser)>0)
                                        @foreach($taggedUser as $user)
                                            <span class="badge badge-warning tooltip-primary" data-toggle="tooltip" data-placement="top" data-original-title="{{$user['FirstName'].' '.$user['LastName']}}">{{substr($user['FirstName'],0,1).substr($user['LastName'],0,1)}}</span>
                                        @endforeach
                                    @endif
