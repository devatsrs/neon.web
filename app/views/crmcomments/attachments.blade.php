            ...
            @foreach($attachementPaths as $index=>$attachement)
                <div class="col-md-2 attachment">
                    <div data-trigger="fileinput" class="fileinput-new thumbnail">
                        ...
                        <p class="text-center"><a href="{{validfilepath($attachement->filepath)}}" alt="{{$attachement->filename}}" target="_blank">{{strlen($attachement->filename)>8?substr($attachement->filename,0,8).'..':$attachement->filename}}</a></p>
                    </div>
