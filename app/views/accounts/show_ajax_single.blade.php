 @if(count($response))
  ...
  <div class="cbp_tmlabel normal_tag">  
              <h2 class="toggle_open" id_toggle="{{$key}}">@if($response->CreatedBy==$current_user_title) You @else {{$response->CreatedBy}}  @endif <span>sent an email to</span> @if($response->EmailTo==$current_user_title) You @else {{$response->EmailTo}}  @endif <br> <p>Subject: {{$response->Subject}}</p></h2>
...
<li id="timeline-{{$key}}" row-id="{{$response->NoteID}}" class="count-li timeline_note_entry">
  ...
  <div class="cbp_tmlabel normal_tag">  
    <a id="edit_note_{{$response->NoteID}}" note-id="{{$response->NoteID}}"  key_id="{{$key}}" class="pull-right edit-deal edit_note_link"><i class="entypo-pencil"></i></a>
            <a id="delete_note_{{$response->NoteID}}" note-id="{{$response->NoteID}}"  key_id="{{$key}}" class="pull-right edit-deal delete_note_link"><i class="fa fa-trash-o"></i></a>
