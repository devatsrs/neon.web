...
class TimezonesController extends BaseController {
    ...
    public function store() {
        $data = Input::all();
        Log::info('TimeZoneController:store ' . print_r($data,true));
        if(!empty($data)){
            $rules = array(
    ...
    public function search_ajax_datagrid_Vendor($type,$id){
        $data = Input::all();
        $data['iDisplayStart'] +=1;
    ...
    public function vednorIndex($id)
    {
        Log::info('search_ajax_datagrid ' . $id);
