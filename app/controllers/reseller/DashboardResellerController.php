...
class DashboardResellerController extends BaseController {
    ...
    public function home() {
        ...
        return View::make('reseller.index',compact('account','original_startdate','original_enddate'));
    ...
    public function invoice_expense_chart(){
        ...
        return View::make('reseller.billingdashboard.invoice_expense_chart', compact('InvoiceExpense','CurrencySymbol'));
    ...
    public function invoice_expense_total(){
        ...
        return View::make('reseller.billingdashboard.invoice_expense_total', compact( 'CurrencyCode', 'CurrencySymbol','TotalOutstanding'));
    ...
    public function monitor_dashboard(){
        ...
        $User 					= 	User::find(Reseller::get_currentUser()->Owner);
        $AccountManager 		= 	$User->FirstName.' '.$User->LastName;
        $AccountManagerEmail 	= 	$User->EmailAddress;
        return View::make('reseller.dashboard',compact('DefaultCurrencyID','original_startdate','original_enddate','isAdmin','newAccountCount','isDesktop','AccountManager','AccountManagerEmail'));
	...
	public function subscriptions(){
		$id=0;
        $companyID = User::get_companyID();
        return View::make('reseller.subscriptions.index', compact(''));
	}
	
   public function subscriptions_ajax_datagrid(){
        $data 	= 	Input::all();        
        $id		=	Reseller::get_accountID();
