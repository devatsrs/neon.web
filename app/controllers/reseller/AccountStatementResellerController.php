...
class AccountStatementResellerController extends \BaseController {
	...
	public function index()
	{
        $id=0;
        $companyID = User::get_companyID();
        $AccountName = Reseller::get_accountName();
        //$UserName = User::get_user_full_name();
        $CompanyName = Company::getName($companyID);
        return View::make('reseller.accountstatement.index', compact('CompanyName','AccountName'));
	}
    ...
    public function exports($type) {
        ...
        self::generateExcel($output, $type);
    }
    ...
    static function generateExcel($account_statement, $type)
    {
        Excel::create('Account Statement', function ($excel) use ($account_statement) {
            $excel->sheet('Account Statement', function ($sheet) use ($account_statement) {
                ...
                self::insertExcelHeaderData($sheet, "A2",'INVOICE OFFSETTING',14 );
                ...
                self::insertExcelHeaderData($sheet, "A4",$account_statement['CompanyName'] . ' INVOICE',12);
                self::insertExcelHeaderData($sheet, "I4",$account_statement['AccountName'] . ' INVOICE',12);
                ...
                //Invoice Out
                self::insertExcelHeaderData($sheet, $Alpha[$columnIndex++] . $RowIndex, 'INVOICE NO');
                self::insertExcelHeaderData($sheet, $Alpha[$columnIndex++] . $RowIndex, 'PERIOD COVERED');
                $InvoiceOutAmountIndex = $columnIndex;
                self::insertExcelHeaderData($sheet, $Alpha[$columnIndex++] . $RowIndex, 'AMOUNT');
                $InvoiceOutDisputeAmountIndex = $columnIndex;
                self::insertExcelHeaderData($sheet, $Alpha[$columnIndex++] . $RowIndex, 'PENDING DISPUTE');

                $columnIndex++;

                //Payment IN
                self::insertExcelHeaderData($sheet, $Alpha[$columnIndex++] . $RowIndex, 'DATE');
                $PaymentInAmountIndex = $columnIndex;
                self::insertExcelHeaderData($sheet, $Alpha[$columnIndex++] . $RowIndex, $account_statement['AccountName'] . ' PAYMENT');
                $PaymentInBalanceIndex = $columnIndex;
                self::insertExcelHeaderData($sheet, $Alpha[$columnIndex++] . $RowIndex, 'BALANCE');
                ...
                self::insertExcelHeaderData($sheet, $Alpha[$columnIndex++] . $RowIndex, $account_statement['CompanyName'] . ' PAYMENT');
                ...
                if (count($account_statement['result']) > 0) {
                    ...
                    foreach ($account_statement['result'] as $rowData) {
                        ...
                        //INVOICE OUT
                        self::insertExcelCellData($sheet, $Alpha[$columnIndex++]   . $RowIndex, $rowData['InvoiceOut_InvoiceNo']);
                        self::insertExcelCellData($sheet, $Alpha[$columnIndex++] . $RowIndex, $rowData['InvoiceOut_PeriodCover']);
                        self::insertExcelCellData($sheet, $Alpha[$columnIndex++] . $RowIndex, $InvoiceOut_Amount);
                        self::insertExcelCellData($sheet, $Alpha[$columnIndex++] . $RowIndex, $InvoiceOut_DisputeAmount);

                        $columnIndex++;


                        //PAYMEMT IN
                        self::insertExcelCellData($sheet, $Alpha[$columnIndex++]   . $RowIndex, $rowData['PaymentIn_PeriodCover']);
                        self::insertExcelCellData($sheet, $Alpha[$columnIndex++] . $RowIndex, $PaymentIn_Amount);
                        self::insertExcelCellData($sheet, $Alpha[$columnIndex++] . $RowIndex, ''); //Balance

                        $columnIndex++;

                        //INVOICE In
                        self::insertExcelCellData($sheet, $Alpha[$columnIndex++]   . $RowIndex, $rowData['InvoiceIn_InvoiceNo']);
                        self::insertExcelCellData($sheet, $Alpha[$columnIndex++] . $RowIndex, $rowData['InvoiceIn_PeriodCover']);
                        self::insertExcelCellData($sheet, $Alpha[$columnIndex++] . $RowIndex, $InvoiceIn_Amount);
                        self::insertExcelCellData($sheet, $Alpha[$columnIndex++] . $RowIndex, $InvoiceIn_DisputeAmount);

                        $columnIndex++;

                        //PAYMEMT OUT
                        self::insertExcelCellData($sheet, $Alpha[$columnIndex++]   . $RowIndex, $rowData['PaymentOut_PeriodCover']);
                        self::insertExcelCellData($sheet, $Alpha[$columnIndex++] . $RowIndex, $PaymentOut_Amount);
                        self::insertExcelCellData($sheet, $Alpha[$columnIndex++] . $RowIndex, ''); //Balance
                ...
                self::insertExcelSummeryData($sheet, $Alpha[$InvoiceOutAmountIndex].$RowIndex, $InvoiceOutAmountTotalFormula);
                ...
                self::insertExcelSummeryData($sheet, $Alpha[$InvoiceOutDisputeAmountIndex].$RowIndex, $InvoiceOutDisputeAmountTotalFormula);
                ...
                self::insertExcelSummeryData($sheet, $Alpha[$PaymentInAmountIndex].$RowIndex, $PaymentInAmountTotalFormula);
                ...
                self::insertExcelSummeryData($sheet, $Alpha[$PaymentInBalanceIndex].$RowIndex, $PaymentInBalanceFormula);
                ...
                self::insertExcelSummeryData($sheet, $Alpha[$InvoiceInAmountIndex].$RowIndex, $InvoiceInAmountTotalFormula);
                ...
                self::insertExcelSummeryData($sheet, $Alpha[$InvoiceInDisputeAmountIndex].$RowIndex, $InvoiceInDisputeAmountTotalFormula);
                ...
                self::insertExcelSummeryData($sheet, $Alpha[$PaymentOutAmountIndex].$RowIndex, $PaymentOutAmountTotalFormula);
                ...
                self::insertExcelSummeryData($sheet, $Alpha[$PaymentOutBalanceIndex].$RowIndex, $PaymentOutBalanceFormula);
                ...
                self::insertExcelHeaderData($sheet, $Alpha[1].$RowIndex, 'BALANCE AFTER OFFSET:',14);

                $BalanceAfterOffsetFormula = '=('.$PaymentInBalanceIndexCell  . '-' . $PaymentOutBalanceIndexCell . ')';
                self::insertExcelSummeryData($sheet, $Alpha[2].$RowIndex, $BalanceAfterOffsetFormula);
...
// Excel functions
    public static function insertExcelCellData(&$sheet, $target_cell, $value)
    {
        ...
        $sheet->cell($target_cell, function ($cell) use ($value) {
            self::formateExcelCell($cell, false);
            $cell->setValue($value);
            $cell->setBackground('#EBF5F2');
        });
