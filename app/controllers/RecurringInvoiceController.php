...
class RecurringInvoiceController extends \BaseController {
    ...
    public function ajax_datagrid($type)
	{
        ...
        if(isset($data['Export']) && $data['Export'] == 1)
		{
            ...
            if($type=='csv'){
                $file_path = CompanyConfiguration::get('UPLOADPATH') .'/RecurringInvoice.csv';
                $NeonExcel = new NeonExcelIO($file_path);
                $NeonExcel->download_csv($excel_data);
            }elseif($type=='xlsx'){
                $file_path = CompanyConfiguration::get('UPLOADPATH') .'/RecurringInvoice.xls';
                $NeonExcel = new NeonExcelIO($file_path);
    ...
    public function ajax_recurringinvoicelog_datagrid($id,$type) {
        ...
        if(isset($data['Export']) && $data['Export'] == 1) {
            ...
            if($type=='csv'){
                $file_path = CompanyConfiguration::get('UPLOADPATH') .'/RecurringInvoice Log.csv';
                $NeonExcel = new NeonExcelIO($file_path);
                $NeonExcel->download_csv($excel_data);
            }elseif($type=='xlsx'){
                $file_path = CompanyConfiguration::get('UPLOADPATH') .'/RecurringInvoice Log.xls';
                $NeonExcel = new NeonExcelIO($file_path);
                $NeonExcel->download_excel($excel_data);
            }
        }
