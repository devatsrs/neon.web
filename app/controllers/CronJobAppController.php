...
class CronJobAppController extends \BaseController {
    ...
    public function delete($id)
    {
        if( intval($id) > 0){
           /* if(!CronJob::checkForeignKeyById($id)) {*/
                try {
                    $result = CronJobApp::find($id)->delete();
					CronJobLogApp::where("CronJobID",$id)->delete();
                   	 if ($result) {
    ...
    public function ajax_load_cron_dropdown(){
        ...
        if(isset($data['CronJobCommandID']) && intval($data['CronJobCommandID']) > 0) {
            $commandconfig = CronJobCommandApp::getConfig($data['CronJobCommandID']);
            ...
            if($CronJobCommand->Command == 'sippyaccountusage'){
                ...
                $services = Service::lists('ServiceName', 'ServiceID');
    ...
    public function cronjob_monitor(){
        ...
        return View::make('cronjob.cronjob_monitor_app', compact('commands','crontab_status','Nodes'));

    }

    public function cronjob_monitor_app(){
    ...
    public function trigger($CronJobID){
        ...
        $Success = false;
        $Server = false;
        $CheckServerStatus = NodesApp::getServersFromCronJob($CronJobID,$CompanyID);
		if(!empty($CheckServerStatus) && count($CheckServerStatus) > 0){
            foreach($CheckServerStatus as $ServerID){
                $CheckServerUp = NodesApp::where(['ServerID' => $ServerID ,'ServerStatus' => '1', 'MaintananceStatus' => '0'])->first();
			...
			return Response::json(array("status" => "failed", "message" => "Please select atleast one node"));
		}
