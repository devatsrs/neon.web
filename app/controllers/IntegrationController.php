		...
		if($data['firstcategory']=='payment'){
			...
			if ($data['secondcategory'] == 'MASAV') {
				...
				if (count($masaDbData) > 0) {
					$SaveData = array("Settings" => json_encode($masaData), "updated_by" => User::get_user_full_name(), 'ParentIntegrationID' => $data['firstcategoryid']);
					IntegrationConfiguration::where(array('IntegrationConfigurationID' => $masaDbData->IntegrationConfigurationID))->update($SaveData);
				} else {
					$SaveData = array("Settings" => json_encode($masaData), "IntegrationID" => $data['secondcategoryid'], "CompanyId" => $companyID, "created_by" => User::get_user_full_name(), 'ParentIntegrationID' => $data['firstcategoryid']);
					IntegrationConfiguration::create($SaveData);
				}
