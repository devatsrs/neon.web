...
class BillingDashboard extends \BaseController {
    ...
    public function VOSAccountBalance_ajax_datagrid($type)
    {
        $data 							 = 		Input::all();
    ...
    public function VOSIP_ajax_datagrid($type){

        $data 							 = 		Input::all();

        $CompanyID 						 = 		User::get_companyID();
        $data['iDisplayStart'] 			+=		1;
        $data['AccountIPSearch'] = !empty($data['AccountIPSearch'])?$data['AccountIPSearch']:'';

        $columns = array('AccountName','RemoteIps','IPType');
        $sort_column = $columns[$data['iSortCol_0']];

        $query = "call prc_getVOSAccountIP(".$CompanyID.",'".$data['AccountIPSearch']."',".( ceil($data['iDisplayStart']/$data['iDisplayLength']) )." ,".$data['iDisplayLength'].",'".$sort_column."','".$data['sSortDir_0']."'";
        if(isset($data['Export']) && $data['Export'] == 1) {
            $excel_data  = DB::connection('sqlsrv')->select($query.',1)');
            $excel_data = json_decode(json_encode($excel_data),true);
            if($type=='csv'){
