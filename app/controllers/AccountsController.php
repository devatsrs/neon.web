...
";
    ...
    // import missing account by compare tblgatewayaccount
    public function import_missing_gatway_account(){
        $gateway = CompanyGateway::getMissingCompanyGatewayIdList();
        return View::make('accounts.importmissingaccount',compact('gateway'));
    }
    ...
    public function check_upload() {
        try {
            ...
            $data = Input::all();
            if (!isset($data['CompanyGatewayID']) || empty($data['CompanyGatewayID'])) {
                return json_encode(["status" => "failed", "message" => 'Please Select a Gateway']);
