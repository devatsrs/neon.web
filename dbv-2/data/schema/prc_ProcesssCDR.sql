CREATE DEFINER=`root`@`localhost` PROCEDURE `prc_ProcesssCDR`(IN `p_CompanyID` INT, IN `p_CompanyGatewayID` INT, IN `p_processId` INT, IN `p_tbltempusagedetail_name` VARCHAR(200), IN `p_RateCDR` INT, IN `p_RateFormat` INT, IN `p_NameFormat` VARCHAR(50))
BEGIN
	...
	UPDATE LocalRMCdr.tblUsageHeader uh
	INNER JOIN tblGatewayAccount ga
		ON ga.GatewayAccountID = uh.GatewayAccountID
		AND ga.CompanyID = uh.CompanyID
		AND ga.CompanyGatewayID = uh.CompanyGatewayID
	...
	DO
		...
		CALL LocalRatemanagement.prc_getCustomerCodeRate(v_AccountID_,v_TrunkID_,p_RateCDR);
	...
	THEN
		...
		DO
			...
			CALL LocalRatemanagement.prc_getCustomerInboundRate(v_AccountID_,p_RateCDR);
