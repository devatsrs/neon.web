...
BEGIN
	...
	THEN
		...
		SET @stm = CONCAT('
		UPDATE   NeonCDRDev.`' , p_tbltempusagedetail_name , '` ud
		LEFT JOIN NeonRMDev.tmp_vcodes_ cr ON cr.Code = ud.area_prefix
		SET buying_cost =
		CASE WHEN  billed_second >= Interval1
		THEN
			(Rate/60.0)*Interval1+CEILING((billed_second-Interval1)/IntervalN)*(Rate/60.0)*IntervalN+IFNULL(ConnectionFee,0)
...
BEGIN
  ...
  END IF;
	...
	DO
		...
		CALL NeonRMDev.prc_getVendorCodeRate(v_AccountID_,v_TrunkID_,p_RateCDR,p_RateMethod,p_SpecifyRate);
...
BEGIN
	...
	INSERT INTO NeonCDRDev.`' , p_tbltempvendorcdrl_name , '` (
		CompanyID,
		CompanyGatewayID,
		GatewayAccountID,
		GatewayAccountPKID,
