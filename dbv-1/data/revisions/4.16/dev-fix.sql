...
CHANGE COLUMN `created_at` `created_at` DATETIME NULL DEFAULT CURRENT_TIMESTAMP AFTER `updated_at`;
...
BEGIN
		...
		INSERT INTO tmp_AccountIDs_ (AccountID,TrunkID)
		SELECT DISTINCT tblVendorRate.AccountID,TrunkID from tblVendorRate
		INNER JOIN tblAccount on tblVendorRate.AccountId = tblAccount.AccountID
		WHERE Status = 1
		ORDER BY  tblVendorRate.AccountID,TrunkID;
...
DROP PROCEDURE IF EXISTS `prc_WSCronJobDeleteOldVendorRate_OLD`;
DELIMITER //
CREATE PROCEDURE `prc_WSCronJobDeleteOldVendorRate_OLD`()
BEGIN
 	 SET SESSION TRANSACTION ISOLATION LEVEL READ COMMITTED;

	-- past and no end date move to archive table.
	...
	INSERT INTO tblVendorRateArchive
		...
		-- DELETE vr
		FROM tblVendorRate vr
		INNER JOIN tblAccount a on a.AccountId = vr.AccountID AND a.Status = 1
		...
		WHERE  -- FIND_IN_SET(vr.AccountId,p_AccountIds) != 0 AND FIND_IN_SET(vr.TrunkID,p_TrunkIds) != 0
					  vr.EndDate is null
						AND vr.EffectiveDate <= NOW()
					-- AND FIND_IN_SET(vr2.AccountId,p_AccountIds) != 0 AND FIND_IN_SET(vr2.TrunkID,p_TrunkIds) != 0
